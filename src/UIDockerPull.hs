...
module UIDockerPull(testUIDockerPull, uiDockerPull, initialDockerImageInfo, DockerImageInfo) where
...
draw f = [C.vCenter $ C.hCenter form <=> C.hCenter help]
    where
        form = B.borderWithLabel (str "Pull Image") $ padTop (Pad 1) $ hLimit 80 $ renderForm f
...
uiDockerPull :: DockerImageInfo -> IO DockerImageInfo
uiDockerPull oldInfo = do
    let buildVty = do
...
testUIDockerPull :: IO ()
testUIDockerPull = do
    newInfo <- uiDockerPull initialDockerImageInfo
