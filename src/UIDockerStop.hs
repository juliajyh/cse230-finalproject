...
draw f = [C.vCenter $ C.hCenter form <=> C.hCenter help]
    where
        form = B.borderWithLabel (str "Stop Container") $ padTop (Pad 1) $ hLimit 80 $ renderForm f
...
uiDockerStop :: DockerContainerInfo -> IO DockerContainerInfo
uiDockerStop oldInfo = do
    let buildVty = do
...
testUIDockerStop :: IO ()
testUIDockerStop = do
    newInfo <- uiDockerStop initialDockerContainerInfo
