...
module UIDockerContainerRm(testUIDockerContainerRm, uiDockerContainerRm, initialDockerContainerInfo, DockerContainerInfo) where
...
draw f = [C.vCenter $ C.hCenter form <=> C.hCenter help]
    where
        form = B.borderWithLabel (str "Remove Container") $ padTop (Pad 1) $ hLimit 80 $ renderForm f
...
uiDockerContainerRm :: DockerContainerInfo -> IO DockerContainerInfo
uiDockerContainerRm oldInfo = do
    let buildVty = do
...
testUIDockerContainerRm :: IO ()
testUIDockerContainerRm = do
    newInfo <- uiDockerContainerRm initialDockerContainerInfo
